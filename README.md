...
[![Codegen probes](https://img.shields.io/badge/probes-133_GREEN_0_RED-brightgreen)](#probe-enumeration-meter)
...
unit tests, all 133 hand-authored probe fixtures, and the gated
...
test suite of 1,285+ functional tests, plus an additional 133-fixture probe
...
| 7 | Full-equivalence umbrella + `make test` / `make test-slow` | FULL-EQUAL 888 / 0 DIFF; **133 probe GREEN / 0 RED**; cmdline-parity 38/0 | 🟡 17 released-corpus diagnostic findings open — pending user sign-off |
...
133 hand-authored fixtures (`csrc/tests/codegen_probes/`) that deliberately
...
against the Python oracle. **133 probes GREEN, 0 RED** across 10
...
# zxbpp + zxbasm + zxbc parse + zxbc codegen + 133-fixture probe series +
...
series (`csrc/tests/codegen_probes/`, **133 fixtures across 10 categories** —
...
reach — **133/133 GREEN**, hand-authored to enforce no silent drift on subtle
...
```
 ...
 Phase 3  ✅  BASIC Frontend — faithful PLY/LALR(1) port
    │         1033/1033 parse-only PASS, 0 false-positives, 133 probes GREEN
    │
 ...
 Phase 7  ✅  Full-equivalence umbrella — `make test` / `make test-slow`
    │         FULL-EQUAL 888 / 0 DIFF; 133 probes GREEN; 132 unit tests GREEN
    │
