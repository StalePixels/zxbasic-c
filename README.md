...
[![Codegen probes](https://img.shields.io/badge/probes-136_GREEN_0_RED-brightgreen)](#probe-enumeration-meter)
...
unit tests, all 136 hand-authored probe fixtures, and the gated
...
test suite of 1,285+ functional tests, plus an additional 136-fixture probe
...
| 7 | Full-equivalence umbrella + `make test` / `make test-slow` | FULL-EQUAL 888 / 0 DIFF; **136 probe GREEN / 0 RED**; cmdline-parity 38/0 | 🟡 14 released-corpus diagnostic findings open — pending user sign-off |
...
136 hand-authored fixtures (`csrc/tests/codegen_probes/`) that deliberately
...
against the Python oracle. **136 probes GREEN, 0 RED** across 10
...
5 BINARY-EQUAL, 10 FRONTEND-EQUAL, 14 open `DIFF-STDERR` findings**
...
# zxbpp + zxbasm + zxbc parse + zxbc codegen + 136-fixture probe series +
...
series (`csrc/tests/codegen_probes/`, **136 fixtures across 10 categories** —
...
reach — **136/136 GREEN**, hand-authored to enforce no silent drift on subtle
...
```
 ...
 Phase 3  ✅  BASIC Frontend — faithful PLY/LALR(1) port
    │         1033/1033 parse-only PASS, 0 false-positives, 136 probes GREEN
    │
 ...
 Phase 7  ✅  Full-equivalence umbrella — `make test` / `make test-slow`
    │         FULL-EQUAL 888 / 0 DIFF; 136 probes GREEN; 132 unit tests GREEN
    │
...
model Claude), directed and supervised by
