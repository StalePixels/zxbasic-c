...
# nextbuild-c: the original script lives in NextBuild/Scripts/ so its BASE_DIR
# is one level up. We live in zxbasic-c/csrc/scripts/, so walk up to a sibling
# _ref/NextBuild checkout (override with NEXTBUILD_DIR env var if elsewhere).
relative_path_to_parent = os.environ.get(
    'NEXTBUILD_DIR',
    os.path.join(os.pardir, os.pardir, os.pardir, '_ref', 'NextBuild'))
...
# nextbuild-c.py — verbatim copy of NextBuild's nextbuild.py, with only the
# `zxbc.main([...])` calls swapped for the C zxbc binary. Everything else
...
try:
    ...
    class nextcreator:  # noqa
        @staticmethod
        def makenex(*a, **k): pass
from datetime import datetime
try:
    import txt2nextbasic
except Exception:
    class txt2nextbasic:  # noqa
        b_start = 0
        @staticmethod
        def main(*a, **k): pass
